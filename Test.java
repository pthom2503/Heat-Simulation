...
public class Test extends TempAdjust implements ActionListener, ItemListener {
	// Private variables of the GUI components
	JTextField tField, bField, lField, rField, dimRowField, dimColField, countField, countFieldCent;
	JButton runButton;
	int dRow, dCol, count;
	...
	public void actionPerformed(ActionEvent e) {
				...
				BigDecimal top, bot = null, left=null, right=null;
				int red;
...
if (!center){
	...
	// normal rounding
	top=top.setScale(2,RoundingMode.HALF_UP);bot=bot.setScale(2,RoundingMode.HALF_UP);left=left.setScale(2,RoundingMode.HALF_UP);right=right.setScale(2,RoundingMode.HALF_UP);
	...
	if(center)
	{
		...
		oldPlateCenter = initialize(oldPlateCenter, top, bot, left, right);
		finalPlate = new BigDecimal[dRow][dCol];
		finalPlate = initialize(newPlateCenter, top, bot, left, right);
	}else
	{
		// creates the plates
		...
		// sets edge temperatures for plates
		oldPlateSides = initialize(oldPlateSides, top, bot, left, right);
		finalPlate = new BigDecimal[dRow + 2][dCol + 2];
		finalPlate = initialize(newPlateSides, top, bot, left, right);
		finalPlate = iterator(oldPlateSides, newPlateSides, count);
	}

	// final display frame
	JFrame frame = new JFrame("Heat Distribution");

	JPanel grid = new JPanel();grid.setLayout(new GridLayout(dRow,dCol));for(
	int i = 1;i<=dRow;i++)
	{
